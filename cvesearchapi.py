...
class CVESearch(object):
    ...
    def __init__(self, base_url: str='https://cve.circl.lu', proxies: MutableMapping[str, str]={}, timeout: Optional[int]=None):
        self.base_url = base_url
        self.session = requests.Session()
        self.session.proxies = proxies
        self.session.headers.update({
            'content-type': 'application/json',
            'User-Agent': 'CVEScan - python wrapper'})
        self.timeout = timeout

    def _http_get(self, api_call, query=None):
        if query is None:
            url = urljoin(self.base_url, f'{self.base_url}/api/{api_call}')
        else:
            url = urljoin(self.base_url, f'{self.base_url}/api/{api_call}/{query}')
